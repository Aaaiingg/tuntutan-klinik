<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Dashboard pembantu tadbir jabatan Kewangan') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded-xl p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <form method="GET" action="{{ route('ptk.dashboard') }}" class="flex gap-2">
    ...
    <form action="{{ route('ptk.tuntutan.lulus', $claim->id) }}" method="POST" onsubmit="return confirm('Anda pasti ingin meluluskan tuntutan ini?')">
        @csrf
        <button type="submit" class="px-3 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600">
            Diluluskan
        </button>
    </form>
    <form action="{{ route('ptk.tuntutan.tidakLengkap', $claim->id) }}" method="POST" onsubmit="return confirm('Anda pasti ingin menolak tuntutan ini?')">
        @csrf
