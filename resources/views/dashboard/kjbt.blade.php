<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <x-dashboard-card title="Tuntutan Disahkan" color="green" :value="$tuntutanDisahkan" />
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-600 text-gray-800 dark:text-white">
                            ...
                            <td class="p-2">{{ $loop->iteration }}</td>
                                    ...
                                    'SAH KPTJKP' => 'bg-blue-100 text-blue-800',
                            ...
                            <td class="p-2 flex items-center space-x-2">
                                @if(!$claim->sahkan_ketua_jabatan)
    <form action="{{ route('kjbt.tuntutan.sahkan', $claim->id) }}" method="POST" onsubmit="return confirm('Anda pasti ingin mengesahkan tuntutan ini?')">
        @csrf
    ...
    @endif
    <form action="{{ route('kjbt.tuntutan.tidakLengkap', $claim->id) }}" method="POST" onsubmit="return confirm('Anda pasti ingin menolak tuntutan ini?')">
        @csrf
        <button type="submit" class="px-3 py-1 text-xs bg-red-500 text-white rounded hover:bg-red-600">
            Ditolak
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="p-4 text-center text-gray-500 dark:text-gray-400">Tiada tuntutan untuk disahkan.</td>
                                </tr>
                            @endforelse
