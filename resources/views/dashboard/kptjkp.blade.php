<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Dashboard Ketua Pembantu Tadbir Jabatan Khidmat Pengurusan') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded-xl p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <form method="GET" action="{{ route('kptjkp.dashboard') }}" class="flex gap-2">
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-600 text-gray-800 dark:text-white">
                            ...
                            <td class="p-2 flex items-center space-x-2">
                                 @if(!$claim->sahkan_kptjkp)
    <form action="{{ route('kptjkp.tuntutan.sahkan', $claim->id) }}" method="POST" onsubmit="return confirm('Anda pasti tuntutan sudah disemak dan ingin megesahkan tuntutan ini?')">
        @csrf
    ...
    @endif
    <form action="{{ route('kptjkp.tuntutan.tidakLengkap', $claim->id) }}" method="POST" onsubmit="return confirm('Anda pasti untuk menolak tuntutan yang tidak lengkap?')">
        @csrf
        <button type="submit" class="px-3 py-1 text-xs bg-red-500 text-white rounded hover:bg-red-600">
            Tidak Lengkap
        </button>
    </form>
    <a href="{{ route('kptjkp.show', $claim->id) }}" class="px-3 py-1 text-xs bg-gray-500 text-white rounded hover:bg-gray-600">
    <i data-lucide="file-text" class="w-5 h-5 text-blue-600"></i>
                            ...
                            {{-- -senarai yang telah disahkan --}}
                            
                            <div class="table-responsive overflow-x-auto">
                                
                    <table class="min-w-full text-sm">
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mt-8 mb-4">
    Senarai Tuntutan Disemak
</h3>
                        <thead>
                            <tr class="bg-gray-100 dark:bg-gray-700 text-left text-gray-600 dark:text-gray-200">
                                <th class="p-2">ID</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-600 text-gray-800 dark:text-white">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="p-4 text-center text-gray-500 dark:text-gray-400">Tiada tuntutan disemak.</td>
                                </tr>
                            @endforelse
